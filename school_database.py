    ...
    def get_student_position_and_points(self, student_id: int, term: str, academic_year: str, form_level: int, school_id: int = None) -> Dict:
        ...
        try:
            with self.get_connection() as conn:
                ...
                rankings = self.get_student_rankings(form_level, term, academic_year, school_id)

                # Only consider students who actually sat for the exam (wrote at least one subject)
                rankings_sat = [r for r in rankings if r.get('total_subjects', 0) > 0]
                total_students = len(rankings_sat)

                # Determine student full name
                student_name = f"{self.get_student_by_id(student_id)['first_name']} {self.get_student_by_id(student_id)['last_name']}"

                # Default position is 'N/A' for students who did not sit
                position = 'N/A'

                # Compute position among students who sat
                target = next((r for r in rankings_sat if r['name'] == student_name), None)
                if target:
                    if form_level >= 3:
                        # For Forms 3&4: count students with better (lower) aggregate points
                        current_points = target.get('aggregate_points')
                        better_count = sum(1 for r in rankings_sat if r.get('aggregate_points', 999) < current_points)
                        position = better_count + 1
                    else:
                        # For Forms 1&2: count passed students with higher average
                        current_average = target.get('average', 0)
                        better_count = sum(1 for r in rankings_sat if r.get('status') == 'PASS' and r.get('average', 0) > current_average)
                        position = better_count + 1
    ...
    def get_student_rankings(self, form_level: int, term: str, academic_year: str, school_id: int = None) -> List[Dict]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # First, get ALL students enrolled in this form level
                if school_id:
                    cursor.execute("""
                        SELECT s.student_id, s.first_name, s.last_name 
                        FROM students s
                        WHERE s.grade_level = ? AND s.status = 'Active' AND s.school_id = ?
                ...
                # For each student, get their marks data (or set defaults if no marks)
                for student_id, first_name, last_name in all_students:
                    ...
                    # Get total subjects written by student (already got from marks_data[2])
                    # total_subjects = marks_data[2] if marks_data else 0
    ...
    def get_subject_position(self, student_id: int, subject: str, term: str, academic_year: str, form_level: int, school_id: int = None) -> str:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Get marks for this specific subject for all students
                subject_marks = []
                students_with_marks = 0
                
                for sid, first_name, last_name in all_students:
                    if school_id:
                        cursor.execute("""
                            SELECT mark FROM student_marks 
                            WHERE student_id = ? AND subject = ? AND term = ? AND academic_year = ? AND school_id = ?
                        """, (sid, subject, term, academic_year, school_id))
                    else:
                        cursor.execute("""
                            SELECT mark FROM student_marks 
                            WHERE student_id = ? AND subject = ? AND term = ? AND academic_year = ?
                        """, (sid, subject, term, academic_year))
                    
                    mark_result = cursor.fetchone()
                    mark = mark_result[0] if mark_result else None
                    subject_marks.append({'student_id': sid, 'mark': mark})
                    
                    # Count only students who actually sat for the paper
                    if mark is not None:
                        students_with_marks += 1
                
                # Use only students who sat for the paper as total
                total_students = students_with_marks
                
                # Sort by mark (descending), treating None as lowest
                subject_marks.sort(key=lambda x: (x['mark'] is None, 1, -x['mark'] if x['mark'] is not None else 0))
                
                # Find student's position
                student_position = 0
                for i, student_mark_data in enumerate(subject_marks):
                    if student_mark_data['student_id'] == student_id:
                ...
                # Get student's mark for tied position calculation
                student_mark = None
                for student_mark_data in subject_marks:
                    if student_mark_data['student_id'] == student_id:
                        student_mark = student_mark_data['mark']
                        break
                
                if student_mark is None:
                    return f"0/{total_students}"
                
                # Count students with better marks (only those with marks)
                better_count = 0
                for student_mark_data in subject_marks:
                    if (student_mark_data['mark'] is not None and 
                        student_mark is not None and 
                        student_mark_data['mark'] > student_mark):
                        better_count += 1
                
                position = better_count + 1
                return f"{position}/{total_students}"
        except Exception as e:
